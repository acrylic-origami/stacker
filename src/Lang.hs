...
type Seg a = (Span, a)
data Segs a = SegFlat [Seg a] | SegTree (STree.STree [Interval (Locd a)] (Locd a))
...
data FnKey = FnNamed Identifier | FnLam Span deriving (Eq, Ord)
...
instance Outputable FnKey where
  ppr (FnNamed i) = O.text "FnNamed" <+> ppr i
